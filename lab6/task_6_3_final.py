...
valid_trunk_values = True

for item in trunk:
    allowed_vlan_list = (trunk[item])
    for vlan in allowed_vlan_list[1:]:
        try:
            int(vlan)
        except ValueError:
            print(f'Неверные значения в списке вланов для интерфейса {item}')
            valid_trunk_values = False
    break
...
for intf, vlans in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith('allowed vlan') and valid_trunk_values == True:
            if vlans[0] == 'add':
                vlans_str = str(vlans).replace("'","").replace(",","")
                print(f' {command}{vlans_str.strip("[]")}')
            elif vlans[0] == 'del':
                vlans_str = str(vlans).replace("'","").replace(",","")
                print(f' {command}{vlans_str.strip("[]")}')
            else:
                vlans_str = str(vlans).replace("'", "").replace(",", "").replace("only","")
                print(f' {command}{vlans_str.strip("[]")}')
        else:
            print(' {}'.format(command))            
